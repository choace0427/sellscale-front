...
import { Group, Box, Collapse, ThemeIcon, Text, UnstyledButton, createStyles, rem, getStylesRef, Flex, Code } from '@mantine/core';
import { IconArrowNarrowRight, IconCalendarStats, IconChevronLeft, IconChevronRight } from '@tabler/icons-react';
...
export function LinksGroup({ icon: Icon, label, mainKey, links }: LinksGroupProps) {
  ...
  const items = (hasLinkList ? links : []).map((link) => (
    <Text<'a'>
      component='a'
      className={cx(classes.link, { [classes.linkActive]: link.key === navTab })}
      href={''}
      key={link.label}
      onClick={(event) => {
    ...
    >
      <Flex>
        <div><link.icon className={classes.linkIcon} size={15} stroke={1.5} /></div>
        <div>{link.label}</div>
      </Flex>
    </Text>
  ));
  ...
  if(!opened && links?.some((link) => link.key === navTab)){
    linkLabel = `${label} – ${links?.find((link) => link.key === navTab)?.label}`;
  }
  ...
  return (
    <>
      <Text<'a'>
        component='a'
        className={cx(classes.baseLink, { [classes.linkActive]: mainKey === navTab || (!opened && links?.some((link) => link.key === navTab)) })}
      ...
      >
        ...
        {mainKey === 'search' && (
          <Code className={classes.version}>{os === 'undetermined' || os === 'macos' ? '⌘' : 'Ctrl'} + K</Code>
        )}
      </Text>
