...
export default function EditProspectModal(props: EditProspect) {
  ...
  const triggerGetProspectByID = async () => {
    ...
    if (result.status === "success") {
      setProspect({
        ...result.data.details,
        ...result.data.email,
        linkedin_url: result.data.li.li_profile,
      });
  ...
  const triggerUpdateProspect = async () => {
    ...
    const result = await patchProspect(
      ...
      prospectInfoForm.values.contractSize
    );
    if (result.status === "success") {
      showNotification({
        title: "Success",
  ...
  const prospectInfoForm = useForm({
    initialValues: {
      ...
      contractSize: prospect?.contract_size || 0,
    },
  });
  ...
  return (
    <Modal
      ...
      title={<Title order={3}>Edit Prospect Details</Title>}
      size="lg"
    >
      ...
      <Flex mb="lg">
        ...
        <Flex direction="column" ml="md" w="100%">
          ...
          <TextInput
            ...
            onChange={(event) =>
              prospectInfoForm.setValues({ title: event.currentTarget.value })
            }
          ...
          <TextInput
            ...
            onChange={(event) =>
              prospectInfoForm.setValues({ email: event.currentTarget.value })
            }
          ...
          <TextInput
            ...
            onChange={(event) =>
              prospectInfoForm.setValues({
                linkedinURL: event.currentTarget.value,
              })
            }
          ...
          <NumberInput
            ...
            size="xs"
            w="90%"
            value={prospectInfoForm.values.contractSize}
          ...
          <Button
            ...
            disabled={
              ...
              prospectInfoForm.values.contractSize === prospect?.contract_size
            }
