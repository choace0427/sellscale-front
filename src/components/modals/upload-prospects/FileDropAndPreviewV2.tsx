...
function getDefaultColumnMappings(fileJSON: any[]) {
  ...
  Object.keys(findBestPreviewRows(fileJSON, 1)[0] || {})
    ...
    .forEach((key) => {
      ...
      const defaultValue = PROSPECT_DB_COLUMNS.includes(convertedKey) ? convertedKey : 'none'
...
export default function FileDropAndPreviewV2(props: FileDropAndPreviewProps) {
  ...
  const checkCanUpload = () => {
    const hasScrapeTarget = Array.from(columnMappings.values()).some((value) => {
    ...
    */
    if (!hasScrapeTarget) {
      failureReasons.push('Please map at least one column to a profile target (such as a LinkedIn URL or email).')
    }
  ...
  const startUpload = async () => {
    ...
    const result = await uploadProspects(+(archetype_id as string), userToken, uploadJSON)
    if (result.status === 'error') {
      ...
      {!fileJSON && (
        ...
        >
          <Flex align={'center'} justify={'center'} gap={80} py={20} style={{ minHeight: 200, pointerEvents: 'none' }}>
            ...
            <Stack>
              <Text color='#878a8c' fw={600}>
                Upload a CSV file with the following columns:
              </Text>
              <Flex bg={'white'} style={{ border: '2px solid #f2f0f3', borderRadius: '8px' }}>
                <Stack py={14} px={24}>
                  <Text
                    bg={'#f9d9d7'}
                    color='#e96c69'
                    ...
                    py={3}
                    size={'sm'}
                    style={{ borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '4px' }}
                  >
                ...
                <Divider size='sm' color='#f2f0f3' orientation='vertical' />
                <Stack py={14} px={24}>
                  <Text
                    bg={'#fae1cf'}
                    color='#ec873a'
      ...
      {fileJSON && (
        <Stack spacing={0} mah={500}>
          ...
          <DataTable
            mih={300}
            highlightOnHover
            columns={determineColumns(columnMappings, setColumnMappings, fileJSON)}
            records={findBestPreviewRows(fileJSON, PREVIEW_FIRST_N_ROWS)}
          />
          <Center pt={20}>
            <HoverCard width={280} position='top' shadow='md' withArrow disabled={checkCanUpload().length === 0}>
              <HoverCard.Target>
                <Button
                  variant='outline'
