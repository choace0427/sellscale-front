...
export function CreateBumpFrameworkContextModal({ context, id, innerProps }: ContextModalProps<CreateBumpFramework>) {
  ...
  const [bumpLengthValue, setBumpLengthValue] = useState(initBumpLengthAsValue ?? 50);
  ...
  const getActiveConvoSubstatusValues = () => {
    ...
    const statuses_avilable = innerProps.dataChannels?.data['LINKEDIN']?.statuses_available;
  ...
  return (
    <Paper>
      ...
      <TextInput
        label={selectedStatus?.includes('ACTIVE_CONVO') ? 'Framework Title' : 'Template nickname'}
      ...
      {/* <Box mx='auto'>
        ...
        <Collapse in={opened}>
          <Textarea
            ...
            placeholder={'This is a description of this framework.'}
            withAsterisk
            autosize
          ...
          {innerProps.showStatus && (
            <Select
              ...
              data={innerProps.dataChannels?.data['SELLSCALE']?.statuses_available?.map((status: string) => {
                return {
                  label: innerProps.dataChannels?.data['SELLSCALE'][status].name,
                  value: status,
                };
              })}
          ...
          <Flex justify='space-between' align='center'>
            ...
            <Flex direction='column' align='flex-end'>
              <Tooltip withinPortal withArrow label='New frameworks must be default enabled.'>
                <span>
                  <Switch
                    pt='md'
                    label='Make default'
