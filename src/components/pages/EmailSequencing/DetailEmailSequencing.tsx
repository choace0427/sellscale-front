...
const DetailEmailSequencing: FC<{
  ...
  useEffect(() => {
    ...
    const activeTemplates = templates.filter(
      (template: EmailSequenceStep) => template.step.active
    );
          ...
          >
            {inactiveTemplates.map((template: EmailSequenceStep, index) => {
              const open_conversion = template.step.times_used
                ? Math.floor(
                    100 *
                      (template.step.times_accepted / template.step.times_used)
                  )
                : "-";
              const reply_conversion = template.step.times_replied
                ? Math.floor(
                    100 *
                      (template.step.times_replied / template.step.times_used)
                  )
                : "-";
              return (
          ...
          <Accordion.Item value="variant-details">
            <Accordion.Control>
              <Flex w="100%" justify={"center"} align={"center"}>
                <Text>
                  {!accordionValue && previewSequence.substring(0, 125) + "..."}
                </Text>
              </Flex>
            </Accordion.Control>
