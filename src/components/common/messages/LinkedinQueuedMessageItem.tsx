import { ActionIcon, Avatar, Badge, Box, Button, Card, Flex, LoadingOverlay, Text, Tooltip, useMantineTheme } from "@mantine/core"
        ...
        <Flex>
          <Flex mr='sm'>
            <Avatar
              ...
              radius="lg"
              size={50}
            >{nameToInitials(props.full_name)}</Avatar>
          </Flex>
          <Flex direction='column'>
            <Text fw='bold' fz='xl'>{props.full_name}</Text>
            <Text fz='md'>{props.title} @ {props.company}</Text>
          </Flex>
        </Flex>
        <Flex>
          <Tooltip
            width={200}
            multiline
            label={"We send LinkedIn messages strategically and randomly throughout the day. We can't reveal the exact time, but rest assured that this message is number " + (props.index + 1) + " in line!"}
          >
            <Badge>
              Position in Queue to be sent: {props.index + 1}
            </Badge>
          </Tooltip>
        </Flex>
      </Flex>
