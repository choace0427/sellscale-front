...
export default function ProspectDetailsResearch(props: { prospectId: number }) {
  return (
    <Card shadow='sm' p='lg' radius='md' mt='md' withBorder>
      ...
      <ProspectDetailsResearchTabs prospectId={props.prospectId} />
    </Card>
  );
}

export function ProspectDetailsResearchTabs(props: { prospectId: number }) {
  ...
  const [isAccountResearchLoading, setIsAccountResearchLoading] = useState(false);

  const fetchAccountResearch = () => {
    const res = fetch(`${API_URL}/research/account_research_points?prospect_id=` + props.prospectId, {
      method: 'GET',
      headers: {
  ...
  const triggerGetResearchPointsHeuristic = async () => {
    setIsLoading(true);
    const result = await getResearchPointsHeuristic(userToken, props.prospectId);
    if(result.status === 'success'){
      setHeuristicResearchArray(result.data);
    } else {
      setHeuristicResearchArray([]);
    }
    setIsLoading(false);
  };
  ...
  return (
    <Tabs defaultValue='personalResearchPoints'>
      <Tabs.List>
        {' '}
        <Tabs.Tab value='personalResearchPoints' icon={<IconUser size='1.25rem' />}>
          Personal
        </Tabs.Tab>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              await displayNotification(
                ...
                async () => {
                  let result = await generateResearchPoints(userToken, props.prospectId, true);
                  return result;
                },
                {
