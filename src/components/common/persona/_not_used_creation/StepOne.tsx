...
export default function StepOne(props: { onChange: (data: { name: string, fileJSON: any[] }) => void }) {

  const creationData = useRecoilValue(personaCreationState);

  const [personaName, setPersonaName] = useState(creationData.persona.archetype);
  const [fileJSON, setFileJSON] = useState<any[]>(creationData.persona.fileJSON);
