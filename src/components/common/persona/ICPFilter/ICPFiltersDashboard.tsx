...
type ProspectFilterState = 'Prospected' | 'Sent Outreach' | 'Accepted' | 'Bumped' | 'Active Convo' | 'Demo' | 'Removed' | 'All Contacts';
...
const ICPFiltersDashboard = (props: ICPFiltersDashboardPropsType) => {
  ...
  const displayProspects = useMemo(() => {
    ...
    filteredProspects = filteredProspects.filter((prospect) => {
      return prospect.full_name.includes(globalSearch) || prospect.title.includes(globalSearch) || prospect.company.includes(globalSearch);
    });
    ...
    >
      ...
      >
        <Flex align={'center'} gap={10}>
          ...
          <Flex align={'center'} gap={10}>
            <Badge color={currentProject?.active ? (currentProject?.num_prospects > 0 ? 'blue' : 'yellow') : 'green'} variant='filled' size='lg'>
          ...
          <Tooltip label={`${currentProject?.num_unused_li_prospects}/${currentProject?.num_prospects} remaining`} withArrow withinPortal color='blue' fw={700}>
            <Flex align={'center'} gap={4}>
              <RingProgress
                ...
                sections={[
                  {
                    value: Math.round(((currentProject?.num_unused_li_prospects || 0) / (currentProject?.num_prospects || 1)) * 1000) / 10,
                    color: 'blue',
                  },
                ]}
              />
              ...
              >
                {Math.round(((currentProject?.num_unused_li_prospects || 0) / (currentProject?.num_prospects || 1)) * 1000) / 10}%
              </span>
            </Flex>
          </Tooltip>
        ...
        <Stack w={'100%'}>
          <Flex wrap={'wrap'} align={'center'} justify={'space-between'}>
            ...
            <Flex w='100%' align={'center'} justify={'space-between'} mt={'sm'}>
              <Button size='sm' onClick={toggleSideBar} variant='outline' color='gray' leftIcon={<IconFilter size={14} />}>
              ...
              <Flex align={'center'} gap={'xs'}>
                ...
                <Flex align={'center'} gap={'xs'}>
                  ...
                  <Select
                    ...
                    data={['Prospected', 'Sent Outreach', 'Accepted', 'Bumped', 'Active Convo', 'Demo', 'Removed', 'All Contacts']}
          ...
          {Object.keys(selectedRows).length > 0 && (
            <Flex justify={'flex-end'} align={'center'} gap={'xs'} mt={'sm'}>
              ...
              <Tooltip withinPortal label="Remove 'Prospected' or 'Sent Outreach' prospects from this campaign.">
                <Button
                  color='red'
                  ...
                  onClick={() => {
                    openConfirmModal({
                      ...
                      children: (
                        <>
                          <Text>
                            Are you sure you want to remove these {Object.keys(selectedRows).length} prospects? This will move them into your Unassigned
                            Contacts list.
                          </Text>
                          <Text mt='xs'>
          ...
          <Paper radius={'8px'}>
            <Flex>
              {sideBarVisible && (
                <Flex gap={'sm'}>
                  <Box
                    ...
                    mr={30}
                  >
                    <SidebarHeader sideBarVisible={sideBarVisible} toggleSideBar={toggleSideBar} isTesting={isTesting} setIsTesting={setIsTesting} />
                    <Stack w={'360px'}>
              ...
              <Box w={'100%'}>
                <GridTabs selectedTab={selectedTab} setSelectedTab={setSelectedTab} icpDashboard={icpDashboard} numProspects={icpProspects.length} />
                <DataGrid
