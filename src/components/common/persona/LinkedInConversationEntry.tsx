  ...
  return (
    <Paper withBorder radius="md" className={classes.comment} p="lg" mb="xs">
      <Group sx={{ position: "relative" }}>
        ...
        {aiGenerated && (
          <AiMetaDataBadge
            ...
            initialMessageStackRankedConfigID={
              initialMessageStackRankedConfigID || 0
            }
            initialMessageStackRankedConfigName={
              initialMessageStackRankedConfigName || ""
            }
      ...
      <TypographyStylesProvider className={classes.body}>
        <Text size="xs">
          <TextWithNewline className={classes.content} breakheight="12px">
            {body}
          </TextWithNewline>
        </Text>
      </TypographyStylesProvider>
...
export function AiMetaDataBadge(props: {
  ...
  const [bumpConversionRate, setBumpConversionRate] = useState<
    number | undefined
  >(undefined);
    ...
    >
      <HoverCard.Target>
        {props.useInfoIcon ? (
          ...
          >
            {props.badgeBFTitle
              ? props.bumpFrameworkTitle && props.bumpFrameworkTitle.length > 20
                ? props.bumpFrameworkTitle.slice(0, 20) + "..."
                : props.bumpFrameworkTitle || "AI"
              : "AI"}
          </Badge>
        )}
      </HoverCard.Target>
      <HoverCard.Dropdown miw="500px">
            ...
            <Flex direction="column" ml="sm">
              ...
              <Text color="dimmed" size="xs" sx={{ lineHeight: 1.2 }} mt="4px">
                {props.bumpFrameworkId
                  ...
                  : "This message was generated using the following information."}
              </Text>
            </Flex>
          </Flex>
          {bumpConversionRate && (
            <Tooltip
              label={
                bumpConversionRate
                  ? `${bumpConversionRate.toFixed(2)}% of prospects converted`
                  : "No conversion data available, yet."
              }
            >
              <Badge>
                {(bumpConversionRate && bumpConversionRate.toFixed(2)) ||
                  "UNKOWN "}
                %
              </Badge>
            </Tooltip>
        ...
        {props.bumpFrameworkId ? (
          <>
            <Text size="sm" mt="md">
              <span style={{ fontWeight: 550 }}>Framework:</span>{" "}
              {props.bumpFrameworkTitle}
            </Text>
            ...
            {props.bumpFrameworkLength && (
              <Badge
                color={valueToColor(theme, props.bumpFrameworkLength)}
                size="xs"
                variant="filled"
              >
                {props.bumpFrameworkLength}
            ...
            {props.initialMessageId || props.cta ? (
              <>
                {props.initialMessageId ? (
                  <>
                    <Text size="sm" mt="sm">
                      <span style={{ fontWeight: 550 }}>CTA:</span>{" "}
                      {props.initialMessageCTAText}
                    </Text>
                    <Text size="sm" mt="sm">
                      <span style={{ fontWeight: 550 }}>Voice:</span>{" "}
                    ...
                    <List>
                      {props.initialMessageResearchPoints?.map(
                        (point, index) => (
                          <List.Item
                            key={index}
                          ...
                          >
                            {props.initialMessageResearchPointTypes &&
                              props.initialMessageResearchPointTypes.length >=
                                index - 1 && (
                  ...
                  <>
                    <Text size="sm" mt="md">
                      <span style={{ fontWeight: 550 }}>Call to Action:</span>{" "}
                      {props.cta}
                    </Text>
                    <Text size="sm" mt="md">
              ...
              <Text size="sm" mt="md" fs={"italic"}>
                This message was generated before June 26th, 2023, prior to
                metadata capture.
              </Text>
            )}
          </>
