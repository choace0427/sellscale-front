...
export default function ICPFitPill(props: {
  ...
  return (
    <Popover position='right' withArrow shadow='md' opened={hovered} withinPortal>
      <Popover.Target>
        <div ref={ref}>
          <ICPFitPillOnly icp_fit_score={props.icp_fit_score} size={props.size} />
        </div>
      </Popover.Target>
      <Popover.Dropdown>
...
export function ICPFitPillOnly(props: { icp_fit_score: number; size?: MantineSize; w?: string }) {
  return (
    <div style={{ position: 'relative' }}>
      <Badge color={icpFitToColor(props.icp_fit_score)} fw={800} size={props.size} w={props.w}>
...
export function ICPFitContents(props: {
  ...
  return (
    <Container w={400}>
      ...
      <Badge color={icpFitToColor(props.icp_fit_score)}>{icpFitToLabel(props.icp_fit_score)}</Badge>
