...
const FAKER_DATA = Array.from({ length: 100 }).map((_, index) => ({
  id: faker.datatype.uuid(),
  cta: faker.lorem.words(25),
...
export default function LinkedInCTAsDrawer(props: {}) {
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: [`query-cta-data`, { page, sortStatus }],
    ...
    >
      <div style={{ height: "90vh" }}>
        <DataTable
          ...
          onRowClick={({ id, cta, percentage, active }) =>
            openModal({
              title: (
              ...
              children: (
                <>
                  ...
                  <Text size="sm" color="dimmed">{cta}</Text>
                </>
              ),
              styles: (theme) => ({
                title: {
