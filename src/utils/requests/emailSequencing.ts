 ...
 * @returns - MsgResponse
 */
export async function createEmailSequenceStep(userToken: string, archetypID: number, overallStatus: string, title: string, template: string, bumpedCount: number | null, setDefault: boolean, substatus: string | null = ""): Promise<MsgResponse> {
  if (!substatus) {
    substatus = "";
  }
 ...
 * @returns - MsgResponse
 */
export async function getEmailSequenceSteps(userToken: string, overallStatuses: string[], substatuses: string[], archetype_ids: number[]): Promise<MsgResponse> {
 ...
 * @returns - MsgResponse
 */
export async function postSequenceStepDeactivate(userToken: string, sequenceStepID: number): Promise<MsgResponse> {

  const response = await fetch(
    `${API_URL}/email_sequence/step/deactivate`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${userToken}`,
