...
export async function getBumpFrameworks(
  ...
  const response = await fetch(
    `${API_URL}/bump_framework/bump?overall_statuses=${overall_statuses_string}&substatuses=${substatuses_string}&archetype_ids=${archetype_ids_string}&exclude_client_archetype_ids=${exclude_archetype_ids_string}&exclude_ss_default=${exclude_ss_default}&unique_only=${unique_only}` +
      (bumpedCount ? `&bumped_count=${bumpedCount}` : '') +
      (include_archetype_sequence_id
        ? `&include_archetype_sequence_id=${include_archetype_sequence_id}`
        : '') +
      (include_assets ? `&include_assets=${include_assets}` : ''),
    {
      method: 'GET',
      headers: {
 ...
 * @returns - MsgResponse
 */
export async function getSingleBumpFramework(
  userToken: string,
  bumpID: number
): Promise<MsgResponse> {
  const response = await fetch(`${API_URL}/bump_framework/bump/${bumpID}`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${userToken}`,
    },
...
export async function getBumpFrameworkMessages(
  userToken: string,
  bumpID: number
): Promise<MsgResponse> {
  const response = await fetch(`${API_URL}/bump_framework/bump-messages?bump_id=${bumpID}`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${userToken}`,
    },
